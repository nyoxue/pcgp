...
int main(int argc, char **argv) {
    ...
    case PCGP_STAGE_BFS: {
        ...
        while (running) {
            ...
            if (!running || (double)(clock_now - clock_prev) / CLOCKS_PER_SEC >= STATE_UPDATE_TIME) {
                {/* print logs */
                    clock_prev      = clock_now;
                    double elapsed  = (double)(clock_now - clock_start) / CLOCKS_PER_SEC;
                    double gps      = elapsed != 0 ? (double)scan.graph_count / elapsed : scan.graph_count;
                    ...
                    rc = fprintf(stderr, "[Stage 0] %.2f%% (%lld/%lld) ETA=%.fs Elapsed=%gs GPS=%g Diam=%d ASPL=%g\n",
                                 ...
                                 eta, elapsed, gps, scan.best_diam, scan.best_aspl);
                    if (rc < 0)
                        goto error;
                    rc = fprintf(log_file, "%llu,%.f,%.f,%d,%g\n",
                                 scan.graph_count, elapsed, eta, scan.best_diam, scan.best_aspl);
                    if (rc < 0)
                        goto error;
                }
            ...
            goto error;
        {
            ...
            while (ele_read = fread(buf, sizeof(*g.s), len, bfs_file)) {
                if (ele_read == len) {
                    ...
                    scan.graph_count++;
                    clock_t clock_now = clock();
                    if ((scan.graph_count == scan.graph_count_max) || (double)(clock_now - clock_prev) / CLOCKS_PER_SEC >= STATE_UPDATE_TIME) {
                        {/* print logs */
                            clock_prev      = clock_now;
                            double elapsed  = (double)(clock_now - clock_start) / CLOCKS_PER_SEC;
                            double gps      = (double)scan.graph_count / elapsed;
                            double eta      = (double)(scan.graph_count_max - scan.graph_count) / gps;
                            double scan_prc = (double)scan.graph_count / scan.graph_count_max * 100.0;
                            int rc = 0;
                            rc = fprintf(stderr, "[Stage 1] %.2f%% (%lld/%lld) ETA=%.fs Elapsed=%gs GPS=%g BISECT_COST=%d\n",
                                         scan_prc, scan.graph_count, scan.graph_count_max,
                                         eta, elapsed, gps, scan.best_bisect_cost);
                            if (rc < 0)
                                goto error;
                        }
                        {/* checkpoint the state file */
